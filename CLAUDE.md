...
**LaTeX diff review modal**: When the agent run completes, `master_cc.py` compares the `.tex` file against a pre-run snapshot. If changed, it reverts the file, stores the proposed edit in `src/core/latex.py::_pending_proposal`, and broadcasts a `tex_edit_proposed` WS event. The frontend shows a side-by-side diff2html modal with Accept/Reject buttons. Accepting writes the new content via `POST /api/latex/accept`; rejecting discards it via `POST /api/latex/reject`. The diff is fetched from `GET /api/latex/diff`.
